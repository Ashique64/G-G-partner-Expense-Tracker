...
import { Trash2, Filter, Loader2, Search, Pencil, X, Save } from 'lucide-react'
...
export default function ExpenseList({ expenses, loading, onDelete, onUpdate }) {
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in">
      {/* Search & Filter Bar */}
      <div className="bg-(--card) border border-(--border) rounded-3xl p-6 shadow-xl flex flex-wrap items-center gap-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select 
            className="bg-(--background) border border-(--border) rounded-xl px-4 py-2.5 text-sm font-bold outline-none focus:border-accent text-(--foreground) appearance-none cursor-pointer"
            value={partnerFilter}
        ...
        <div className="ml-auto bg-accent/10 border border-accent/20 rounded-2xl px-6 py-3 text-right">
          <span className="text-[10px] text-accent uppercase block font-black tracking-[0.2em] mb-1 opacity-70">Total in View</span>
          <span className="text-2xl font-black text-(--foreground) tracking-tight">₹{totalAmount.toLocaleString('en-IN')}</span>
        </div>
      </div>

      {/* Expenses Table */}
      <div className="overflow-hidden bg-(--card) border border-(--border) rounded-3xl shadow-2xl">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-(--border)">
              {filteredExpenses.length === 0 ? (
                <tr>
                  <td colSpan="6" className="px-8 py-20 text-center text-(--muted-foreground) italic font-medium">No transactions found matching your criteria.</td>
                </tr>
              ) : (
                    ...
                    <td className="px-8 py-5 text-center whitespace-nowrap">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button 
                          ...
                          className="p-2 text-(--muted-foreground) hover:text-red-500 hover:bg-red-500/20 rounded-xl transition-all shadow-sm hover:shadow-red-500/10"
                        >
                          <Trash2 size={18} />
                        </button>
                      </div>
          ...
          <div className="bg-(--card) border border-(--border) w-full max-w-lg rounded-4xl p-8 shadow-2xl relative z-10 animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleUpdate} className="space-y-6">
              <div className="grid grid-cols-1 gap-4">
                ...
                <div>
                  <label className="text-[10px] font-black text-(--muted-foreground) uppercase tracking-widest mb-2 block">Amount (₹)</label>
                  <input 
                    type="number" 
                ...
                <div>
                  <label className="text-[10px] font-black text-(--muted-foreground) uppercase tracking-widest mb-2 block">Note</label>
                  <textarea 
